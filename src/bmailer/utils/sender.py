...
class EmailSender:
    ...
    def _starttls(self, sock):
        try:
            ...
            # Wrap the socket
            return context.wrap_socket(sock, server_hostname=EMAIL_CONFIG["mta_server"])
    ...
    def _authenticate(self, sock):
        try:
            ...
            self._smtp_command(
                sock,
                base64.b64encode(EMAIL_CONFIG["smtp_username"].encode()).decode(),
                334,
            )
            self._smtp_command(
                sock,
                base64.b64encode(EMAIL_CONFIG["smtp_password"].encode()).decode(),
                235,
            )
        except Exception as e:
        ...
        for attempt in range(self.retries):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    logging.info(
                        f"Connecting to {EMAIL_CONFIG['mta_server']}:{EMAIL_CONFIG['mta_port']}"
                    )
                    ...
                    # Send EHLO
                    ehlo_response = self._smtp_command(
                        sock, f"EHLO {socket.gethostname()}"
                    )
                    ...
                    # Send email
                    self._smtp_command(
                        sock, f"MAIL FROM:<{EMAIL_CONFIG['sender_email']}>"
                    )
